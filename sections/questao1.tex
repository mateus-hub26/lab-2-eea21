...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/EsquematicoExercicio1.png}
    \caption{Diagrama esquemático do DEMUX 1x4 (Portas NOR).}
\end{figure}

O diagrama esquemático soluciona a restrição do projeto de utilizar exclusivamente portas NOR mediante a aplicação do Teorema de De Morgan. A dupla negação das expressões canônicas originais converteu algebricamente os produtos lógicos em somas negadas, permitindo que a seleção e o roteamento do sinal de entrada sejam realizados apenas com esta porta lógica universal.

\subsection{Diagrama de Temporização}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{fig/SimuExercicio1.png}
    \caption{Simulação funcional (diagrama de temporização) do DEMUX 1x4.}

\end{figure}

 A simulação funcional mostra o sucesso do circuito ao comprovar que a adaptação para portas NOR manteve a integridade lógica do demultiplexador. O diagrama evidencia que a forma de onda da entrada de dados é replicada fidedignamente única e exclusivamente na saída $Y_i$ ditada pelas variáveis de seleção naquele instante, confirmando o roteamento exato.
